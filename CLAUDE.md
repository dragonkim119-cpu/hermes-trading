...
### 5. Hermes가 heartbeat.json 타임스탬프를 UTC/KST로 계속 헷갈림
- **증상**: `date -d @<ts>`로 변환한 시각이 사용자가 말하는 "지금"과 몇 시간씩 차이난다며
  하트비트가 오래된 것 아니냐고 반복해서 의심함. 실제로는 몇십 초~몇 분 전으로 항상 정상이었음.
- **원인**: WSL 호스트의 `date -d`는 KST를 출력하지만, Hermes의 실제 명령 실행은 (③번 문제를
  고친 뒤에도) **Docker 샌드박스 컨테이너 안**에서 도는데, 그 컨테이너의 기본 타임존은 UTC임.
  즉 같은 `date -d @<ts>` 명령이라도 어디서 실행되느냐에 따라 KST로 나올 수도, UTC로 나올 수도
  있어서 신뢰할 수 없었음. (처음엔 "출력이 KST일 것"이라고 브리핑했다가, 실제로는 컨테이너에서
  UTC로 나온다는 게 확인되어 다시 고침.)
- **해결**: `TZ='Asia/Seoul' date -d @<ts>` 처럼 **TZ를 항상 명시**하도록 브리핑 수정. 실행 환경의
  기본 타임존에 의존하지 않는 방식이라 컨테이너/호스트 어디서 실행되든 결과가 같음.
- **교훈**: 이 환경은 레이어가 여러 개(WSL 호스트 / Hermes Docker 샌드박스)라 "여기서 되니까
  저기서도 되겠지"라는 가정이 자주 틀림 — 시간대, PATH, 파일시스템 마운트 전부 레이어마다
  따로 확인해야 함.
